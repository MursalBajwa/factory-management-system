...
if (isset($_GET['debt_id'])) {
    ...
    try {
        ...
        // Fetch debt details for the selected debt_id
        $debt_id = $_GET['debt_id'];
    
        // Query to get debt details
        ...
        if ($stmt = $conn->prepare($debt_query)) {
            $stmt->bind_param("i", $debt_id); // Bind the debt_id as an integer
            $stmt->execute();
            ...
            if ($result->num_rows > 0) {
                ...
                echo "<p>No details found for the selected debt.</p>";
            }
            $stmt->close();
        } else {
